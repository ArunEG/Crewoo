...
def get_project_as_pdf(request,data):
	...
	response = PDFTemplateResponse(request=request,\
		...
		cmd_options={ 'encoding': 'utf8','quiet': True}
	)
	return response
...
def get_project_as_excel(project):
	...
	work_sheet.write(row_num, 1, str(strip_tags(project.description)), insertcol)
	work_sheet.write(row_num, 2, project.start_date.strftime('%d-%m-%Y %H:%M'), insertcol)
	work_sheet.write(row_num, 3, project.end_date.strftime('%d-%m-%Y %H:%M'), insertcol)
